...
public class MainApp {
    ...
    private static void showcaseOrientationToolkit() {
        ...
        QAzimuth outboundLeg = new QAzimuth(new QDegree(45.0));
    ...
    private static void showcaseFlightLegComputation() {
        ...
        QSpeed cruise = new QSpeed(820.0, ESpeeds.KILOMETERS_PER_HOUR);
        QDuration legDuration = new QDuration(1.8, ETimes.HOUR);
        double cruiseMeters = cruise.inMeterPerSecond() * legDuration.inSecond();
        QLength cruiseSegment = new QLength(cruiseMeters, ELengths.METER);
        System.out.println("Cruise segment (km): " + cruiseSegment.inKilometer());

        QLength taxiOut = new QLength(8.0, ELengths.KILOMETER);
        QLength totalPath = cruiseSegment.add(taxiOut);
        System.out.println("Total path incl. taxi (km): " + totalPath.inKilometer());

        QAltitude requestedAltitude = new QAltitude(120_000.0, ELengths.METER);
        System.out.println("Requested altitude after clamp (m): " + requestedAltitude.getValue());

        QRange radarRange = new QRange(15_000_000.0, ELengths.METER);
        System.out.println("Radar range capped to (km): " + radarRange.inKilometer());

        QArea corridor = cruiseSegment.toArea(new QLength(2.5, ELengths.KILOMETER));
    ...
    private static void showcaseTemperatureProfile() {
        ...
        QCelsius cabin = new QCelsius(22.0);
        QFahrenheit cabinF = new QFahrenheit(cabin.getValue() * 9.0 / 5.0 + 32);
        QKelvin cabinK = new QKelvin(cabin.inKelvin());
